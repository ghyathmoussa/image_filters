...
// struct to save the pgm and sobel data
typedef struct pgm{
    char pgm_type[3];
    int width;
    int height;
...
// write to pgm file
void write_image(char* file_name,PGM *p,int dir){
    int i,j, max_val = 0;
    FILE *fp;
    printf("Writing....\n");
    ...
    if(dir == 1){
        ...
        printf("can not write to file please reset the direction!!\n");
    }
    fclose(fp);
}
...
// start sobel filter
// each X and Y 
void sobel_edge_det(PGM* p,char* file_out,int dir){
    int i,j,gx,gy;
    int kernal_x[3][3] = {
        {-1,0,1},
        {-2,0,2},
    ...
    output->gx_data = malloc((output->height) * sizeof(int*));
    output->gy_data = malloc((output->height) * sizeof(int*));

    for(i=0;i<output->height;i++){
        output->data[i] = malloc((output->width) * sizeof(int));
        output->gx_data[i] = malloc(output->width * sizeof(int));
        output->gy_data[i] = malloc(output->width * sizeof(int));
    }
    
    for(i=0;i<p->height;i++){
        for(j=0;j<p->width;j++){
            output->data[i][j] = p->data[i][j];
            output->gx_data[i][j] = p->data[i][j];
            output->gy_data[i][j] = p->data[i][j];
        }
    }
    /* End Copy */
    ...
    for(i=1;i<output->height-2;i++){
        for(j=1;j<output->width-2;j++){
            gx = conv(output,kernal_x,i,j);
            gy = conv(output,kernal_y,i,j);
            // printf("gx: %d",gx);
            output->data[i][j] = sqrt(gx*gx + gy*gy);
            output->gx_data[i][j] = gx;
            output->gy_data[i][j] = gy;

        }
    }
    // // normalize data
    normalize(output->width,output->height,output->data);
    normalize(output->width,output->height,output->gx_data);
    normalize(output->width,output->height,output->gy_data);
    write_image(file_out,output,dir);
